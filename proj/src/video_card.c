...
int vg_set_mode(short mode){ //test for 0x105 fails because get_mode_info returns 0 when it shouldnt. When we implement our own function, we must check and return something other than 0
    ...
    unsigned int vram_size = mem_info.XResolution * mem_info.YResolution * bytesPerPixel;  /* VRAM's size, but you can use
...
int colorPixel(unsigned x, unsigned y, unsigned long color){
    ...
    if ((int)x >= h_res || (int)y>= v_res){
        printf("Out of bounds\n");
        return 1;
    }
    //int bytes_per_pixel = 2;
    char* pixel_address=video_mem+(x+y*h_res)*bits_per_pixel / 8;

    *pixel_address = (uint32_t)color;
